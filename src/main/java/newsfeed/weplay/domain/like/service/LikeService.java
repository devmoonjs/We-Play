...
public class LikeService {
    ...
    public void likePost(Long postId, AuthUser authUser) {
        ...
        } //이미 좋아요한 이력이 있는 게시글이면 좋아요 못하게 방지
    ...
    public void deleteLikePost(Long postId, AuthUser authUser) {
        ...
        Likes like = likesRepository.findByUserAndPost(user, post).orElseThrow(() -> new EntityNotFoundException("좋아요를 하지 않았습니다."));
    ...
    public void likeComment(Long commentId, AuthUser authUser) {
        User user = userRepository.findById(authUser.getUserId()).orElseThrow(() -> new EntityNotFoundException("해당 유저가 존재 하지 않습니다."));
        Comment comment = commentRepository.findById(commentId).orElseThrow(() -> new EntityNotFoundException("해당 댓글이 존재 하지 않습니다."));
